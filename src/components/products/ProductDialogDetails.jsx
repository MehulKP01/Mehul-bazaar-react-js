...
const ShoppingCartOutlinedIcon = lazy(() =>
  import("@mui/icons-material/ShoppingCartOutlined")
);
const ShoppingBagOutlinedIcon = lazy(() =>
  import("@mui/icons-material/ShoppingBagOutlined")
);
...
function ProductDialogDetails(props) {
  ...
  const initialVariant = useMemo(() => {
    ...
    return product?.variations?.find(
      (variation) => variation.dimensions && variation.dimensions.length > 0
    );
  }, [product]);
  //state for variant
    ...
    <>
      ...
      <Box pt={1} mb={1}>
        <Box display={"flex"} gap={1} alignItems={"center"}>
          ...
          {regularPrice ? (
            <>
              <H4 color="grey.600" fontWeight="600" mb={0.5} lineHeight="1">
                MRP <del>{currencyFormat(regularPrice, currency)}</del>
              </H4>
              <H4
              ...
              >
                (
                {calculateDiscountPercentage(salePrice, regularPrice, currency)}
                % OFF)
              </H4>{" "}
            </>
          ) : (
                ...
                <>
                  {isAttributeItemSelected(attItem?._id) && (
                    <Chip
                      label={attItem?.name}
                      onClick={(e) => {
                        onAttItemChange(att?._id, attItem?._id);
                      }}
                      sx={{
                        borderRadius: "4px",
                        mr: 1,
            ...
            {isLoading ? (
              ...
              >
                <ShoppingCartOutlinedIcon
                  sx={{ fontSize: isMobile ? "1.2rem" : "1.8rem" }}
                />
                <Typography
                  sx={{
        ...
        <>
          <FlexBox alignItems="center">
            <Button
              ...
              disabled={isLoading}
              onClick={(e) =>
                handleChangeQtyCart1((cartProduct?.quantity ?? 0) - 1)
              }
            >
